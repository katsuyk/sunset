...
if(isset($_POST['submitted'])) {
    ...
    if(!isset($hasError)) {
        ...
        $subject = __('From ','sunset').$name;
        $body = __('Name: ','sunset').$name."\n".__('Email: ','sunset').$email."\n".__('Comments: ','sunset').$comments;
        $headers = __('From: ','sunset') .$name. ' <'.$emailTo.'>' . "\r\n" . __('Reply-To:','sunset') .$name. '<'.$email.'>';
						...
						<?php the_content(); ?>
				          ...
				          <?php if(isset($emailSent) && $emailSent == true) { ?>
				                <div class="alert alert-success" role="alert">
				                    <p><?php _e('Thanks, your email was sent successfully.', 'sunset'); ?></p>
				                </div>
				            <?php } else { ?>
				                ...
				                <?php if(isset($hasError) || isset($captchaError)) { ?>
				                    <div class="alert alert-danger alert-dismissible" role="alert">
									  ...
									  <strong><?php _e('Error!', 'sunset'); ?></strong> <?php _e('Please try again.', 'sunset'); ?>
									</div>
				                <?php } ?>

						        <form action="<?php the_permalink(); ?>" id="contactForm" method="post">
						            <div class="form-group <?php if(isset($nameError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="contactName"><?php _e('Name', 'sunset'); ?></label>
						               ...
						               <div class="form-group <?php if(isset($emailError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="email"><?php _e('Email', 'sunset'); ?></label>
						                ...
						                <div class="form-group <?php if(isset($commentError)) { echo "has-error has-feedback"; }?>">
						                    <label class="control-label" for="commentsText"><?php _e('Message', 'sunset'); ?></label>
						               ...
						               <div class="form-actions">
						                    <button type="submit" class="btn btn-primary"><?php _e('Send Email', 'sunset'); ?></button>
